...
class ConfigurationClassParser {
	...
	protected void processConfigurationClass(ConfigurationClass configClass, Predicate<String> filter) throws IOException {
		...
		if (existingClass != null) {
			if (configClass.isImported()) {
				if (existingClass.isImported()) {
					// 如果要处理的配置类config class在已经分析处理的配置类记录中已存在，合并两者的importBy属性
		...
		// 如果无法理解，可以把它当作一个黑盒，不影响看spring源码主流程
	...
	protected final SourceClass doProcessConfigurationClass(
		...
		if (configClass.getMetadata().isAnnotated(Component.class.getName())) {
			...
			// 递归处理内部类，因为内部类也是一个配置类，配置类上有@configuration注解，该注解继承@Component，if判断为
			processMemberClasses(configClass, sourceClass, filter);
		}

		// Process any @PropertySource annotations
		// 如果配置类上加了@PropertySoruce注解，那么就解析加载properties文件，并将属性添加到spring上下文中
		...
		if (!componentScans.isEmpty() &&
			...
			for (AnnotationAttributes componentScan : componentScans) {
				...
				// 比如 basePackages = com.xxx.xx 那么在这一步会扫描出这个包以及子包下的class，然后将其解析成bean
				// (BeanDefinition可以理解为等价与BeanDefinitionHolder)
				Set<BeanDefinitionHolder> scannedBeanDefinitions =
						this.componentScanParser.parse(componentScan, sourceClass.getMetadata().getClassName());
				// Check the set of scanned definitions for any further config classes and parse recursively if needed
				// 通过上一步扫描包 com.xxx.xx，有可能扫描出来的bean中可能也添加了@ComponentScan或者@ComponentScans
				// 所以这里需要循环遍历一次，进行递归parse，继续解析，知道解析出的类全部没有@ComponentScan或者@ComponentScans注解
